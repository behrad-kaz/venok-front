// components/dashboard/departments/AddDepartmentModal.tsx
"use client";

import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { X, User, Building2, FileText, Power, ChevronDown } from "lucide-react";
...
export default function AddDepartmentModal({
  ...
  const handleSubmit = () => {
    if (!departmentName || !selectedManager) return;
            ...
            <div className="bg-[#0D1B17] border border-[#59D8C3]/20 rounded-2xl overflow-hidden shadow-2xl">
              ...
              <div className="flex items-center justify-between p-5 border-b border-[#59D8C3]/20 bg-gradient-to-r from-[#1a3833]/30 to-transparent">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-xl font-bold text-white">افزودن دپارتمان</h2>
                </div>
              ...
              <div className="p-5 space-y-5">
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="w-full bg-[#12251F] border border-[#59D8C3]/20 rounded-lg p-3 text-right text-white flex items-center justify-between focus:outline-none focus:border-[#59D8C3] transition-colors"
                  >
                    ...
                    <div className="absolute top-full left-0 right-0 mt-2 bg-[#0D1B17] border border-[#59D8C3]/20 rounded-lg overflow-hidden shadow-xl z-50">
                      {managers.map((manager) => (
                        <button
                          ...
                          className="w-full text-right px-4 py-3 hover:bg-[#12251F] transition-colors"
                        >
                ...
                <div>
                  <label className="flex items-center gap-2 text-sm text-gray-300 font-medium mb-2">
                    <FileText className="w-4 h-4 text-[#59D8C3]" />
                    توضیحات
                  </label>
                  <textarea
                    value={description}
                    onChange={(e) => setDescription(e.target.value)}
                    placeholder="توضیح کوتاهی درباره این دپارتمان..."
                    rows={3}
                    className="w-full bg-[#12251F] border border-[#59D8C3]/20 rounded-lg p-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#59D8C3] transition-colors resize-none"
                  />
                </div>
