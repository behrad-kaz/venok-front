// components/dashboard/reports/ReportCards.tsx
"use client";

import { motion } from "framer-motion";
import { MessageCircle, CheckCircle, Clock, TrendingUp } from "lucide-react";

interface ReportCardsProps {
  stats: {
    totalTickets: number;
    solvedTickets: number;
    avgFirstResponse: string;
    avgResolutionTime: string;
    resolutionRate: number;
  };
}

export default function ReportCards({ stats }: ReportCardsProps) {
  const cards = [
    { id: 1, title: "کل گفتگوها", value: stats.totalTickets, change: "+۱۲٪", trend: "up", icon: MessageCircle, color: "#59D8C3" },
    { id: 2, title: "گفتگوهای حل‌شده", value: stats.solvedTickets, change: "+۸٪", trend: "up", icon: CheckCircle, color: "#5BE0A8" },
    { id: 3, title: "میانگین زمان اولین پاسخ", value: stats.avgFirstResponse, change: "-۲٪", trend: "down", icon: Clock, color: "#F2B84B" },
    { id: 4, title: "میانگین زمان حل گفتگو", value: stats.avgResolutionTime, change: "-۵٪", trend: "down", icon: Clock, color: "#F2B84B" },
    { id: 5, title: "نرخ حل‌شدن گفتگوها", value: `${stats.resolutionRate}٪`, change: "+۳٪", trend: "up", icon: TrendingUp, color: "#59D8C3" },
  ];

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
      {cards.map((card, index) => (
        <motion.div
          key={card.id}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: index * 0.1 }}
          className="p-5 rounded-2xl bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.1)] hover:border-[rgba(89,216,195,0.3)] transition-all"
        >
          <div className="flex items-start justify-between mb-3">
            <div className="w-11 h-11 rounded-xl bg-[rgba(89,216,195,0.08)] border border-[rgba(89,216,195,0.15)] flex items-center justify-center">
              <card.icon className="w-5 h-5" style={{ color: card.color }} />
            </div>
            <div className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium bg-[rgba(89,216,195,0.08)] text-[#59D8C3]">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <polyline points={card.trend === "up" ? "18 15 12 9 6 15" : "6 9 12 15 18 9"} />
              </svg>
              <span>{card.change}</span>
            </div>
          </div>
          <div className="space-y-1">
            <p className="text-2xl font-bold text-white">{card.value}</p>
            <p className="text-sm text-gray-500">{card.title}</p>
          </div>
        </motion.div>
      ))}
    </div>
  );
}