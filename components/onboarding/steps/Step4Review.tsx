...
interface Step4ReviewProps {
  ...
  departments?: Department[];  // تغییر به optional
  members?: Member[];          // تغییر به optional
...
export default function Step4Review({ 
  formData, 
  departments = [],  // مقدار پیش‌فرض
  members = [],      // مقدار پیش‌فرض
  logoPreview,
  onBack 
}: Step4ReviewProps) {
  // آمارها با بررسی وجود departments
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            ...
            <div>
              ...
              <p className="text-white font-medium" dir="ltr">{formData?.phone || "-"}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">ایمیل پشتیبانی</p>
              <p className="text-white font-medium" dir="ltr">{formData?.email || "-"}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">دامنه سایت</p>
              <p className="text-white font-medium" dir="ltr">{formData?.domain || "-"}</p>
            </div>
          </div>
        </div>
      </div>
